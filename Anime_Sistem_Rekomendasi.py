...
"""### Hasil Proses :
🔍 Dataset Loading & Preparation Summary
🚀 Proses dimulai dengan mengunduh dan memuat dua dataset utama yang dibutuhkan dalam proyek sistem rekomendasi anime:

📥 Anime Dataset

Berhasil diunduh dari sumber: /kaggle/input/anime-recommendation-database-2020

Dataset ini berisi informasi lengkap tentang anime, termasuk judul, genre, skor, studio, dan atribut lainnya.

Jumlah entri: 17.562 baris dan 35 kolom

📥 Ratings Dataset

Diunduh dari Kaggle API: https://www.kaggle.com/api/v1/datasets/download/marlesson/myanimelist-dataset-animes-profiles-reviews

Dataset ini mencakup ulasan dan penilaian pengguna terhadap anime.

Jumlah entri: 192.112 baris dan 7 kolom

Lokasi ekstraksi: /root/.cache/kagglehub/datasets/marlesson/...

✅ Kedua dataset berhasil dimuat tanpa error.
🎉 Semua data siap digunakan untuk tahap eksplorasi, preprocessing, dan pembangunan model rekomendasi.
...
**Hasil**: ✅ Berhasil - TF-IDF matrix berhasil dibuat dengan shape (17,562 x 47) merepresentasikan setiap anime dengan 47 genre features
...
"""### Hasil Proses :
Berikut penjelasan untuk data contoh dari dataset **ratings dengan review dan metadata anime** yang bisa kamu masukkan ke README atau laporan:

---

### 📊 Sample Data Preview – Ratings & Reviews Dataset

Berikut adalah cuplikan data dari dataset rating dan ulasan pengguna terhadap anime. Dataset ini tidak hanya mencakup skor numerik, tetapi juga informasi kontekstual yang kaya dari review pengguna:

| Kolom      | Deskripsi                                                                                             |
| ---------- | ----------------------------------------------------------------------------------------------------- |
| `user_id`  | ID unik pengguna yang memberikan review.                                                              |
| `profile`  | Username pengguna di MyAnimeList.                                                                     |
| `anime_id` | ID unik untuk setiap anime yang direferensikan.                                                       |
| `text`     | Ulasan teks lengkap dari pengguna, biasanya mencakup pendapat pribadi.                                |
| `rating`   | Skor keseluruhan dari pengguna untuk anime tersebut (skala 1-10).                                     |
| `scores`   | Dictionary berisi skor per-aspek: `Overall`, `Story`, `Animation`, `Sound`, `Character`, `Enjoyment`. |
| `link`     | Tautan langsung ke halaman review pengguna di MyAnimeList.                                            |
| `name`     | Judul anime yang diulas.                                                                              |
| `Genres`   | Daftar genre yang melekat pada anime tersebut.                                                        |

📌 **Contoh**:
Salah satu entri menunjukkan pengguna `DesolatePsyche` memberikan anime *Gintama.* skor **8**, lengkap dengan review dan skor aspek-aspek lain seperti cerita, animasi, dan karakter.

---

### 💡 Insight Awal

* Dataset ini sangat cocok untuk membangun sistem rekomendasi berbasis **Collaborative Filtering** dan **Content-Based Filtering** karena:

  * Terdapat interaksi eksplisit (rating).
  * Tersedia ulasan teks dan genre yang bisa diolah untuk fitur konten.
  * Ada aspek detail seperti `scores` dan `profile`, yang bisa dimanfaatkan untuk personalisasi lebih lanjut.
